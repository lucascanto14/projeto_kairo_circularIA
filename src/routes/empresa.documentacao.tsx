  ...
  return <AppShell title="Verificação da Empresa e Responsável Técnico" subtitle="Etapa obrigatória para anunciar, negociar, enviar propostas e concluir operações.">
    <div className="grid gap-6 xl:grid-cols-[1fr_330px]">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-6 space-y-3">{([{ key: "use", text: "Aceito os termos de uso" }, { key: "data", text: "Aceito a política de tratamento de dados" }] as const).map((item) => <label key={item.key} className="flex items-center gap-3 text-sm"><input type="checkbox" checked={terms[item.key]} onChange={(e) => setTerms((s) => ({ ...s, [item.key]: e.target.checked }))} className="size-4 accent-primary" />{item.text}</label>)}</div>
      </div>
